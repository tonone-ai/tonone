...
Orientation map for this repo — 100 agents, 10 teams, 422 skills across `team/`. Read this first, then drill into the specific `agents/<name>.md` or `team/<agent>/skills/<skill>/SKILL.md` the task needs. Don't crawl the whole tree; use the indexes below.
...
| `docs/skill-index.json` | name, agent, team, description, path — one row per skill | 422  | `scripts/gen-skill-index.py` (reads `team/*/skills/*/SKILL.md` frontmatter) |
...
3. **Do a full resync, not a diff-and-patch** — going from 23→100 agents and 138→422 skills, a merge is more error-prone than regenerating `.planning/skills-data.json` and `.planning/hooks-data.json` from scratch against the two indexes above.
...
- **What we borrowed from which upstream plugin, and at which version** → `docs/upstream.md` (pins in `docs/upstream.json`, drift check via `scripts/check-upstream.py`)
