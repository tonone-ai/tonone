...
Orientation map for this repo — 100 agents, 10 teams, 425 skills across `team/`. Read this first, then drill into the specific `agents/<name>.md` or `team/<agent>/skills/<skill>/SKILL.md` the task needs. Don't crawl the whole tree; use the indexes below.
...
| `docs/skill-index.json` | name, agent, team, description, path — one row per skill | 425  | `scripts/gen-skill-index.py` (reads `team/*/skills/*/SKILL.md` frontmatter) |
...
3. **Do a full resync, not a diff-and-patch** — going from 23→100 agents and 138→425 skills, a merge is more error-prone than regenerating `.planning/skills-data.json` and `.planning/hooks-data.json` from scratch against the two indexes above.
