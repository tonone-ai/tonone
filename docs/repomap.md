...
- **Root `skills/` mirror is incomplete.** Root has 192 skill dirs; `team/*/skills/` has 421. Only Engineering + Product + part of Operations ever got mirrored — Legal, Design, Data Science, Security Operations, Developer Experience, Infrastructure Specialist, and AI Operations (7 of 10 teams) have zero skills copied to root `skills/`. `docs/skill-index.json` reads `team/*/skills/` directly so it's unaffected, but anything still reading root `skills/` as "the catalog" (old `docs/sitemap.md`, platform's sync) only sees 3 of 10 teams.
