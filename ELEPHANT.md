...
2026-09-20 23:51 : built §3.1 go from docs/jev-browser-eval.md against third candidate — browser-use/jev-ultrafast (MIT, Python, pinned commit 1231850, not on PyPI): lib/jev-ultrafast/cli.py preflight+explore, runner.py only file importing driver, new skill /proof-explore, proof-e2e Step 0 reads transcript — @fatih
[!!] 2026-09-20 23:51 : jev-ultrafast attaches to user's REAL Chrome profile via Browser Harness (not a fresh cookieless Chromium like jev-browser) — real sessions reachable, so host allowlist is deny-default: localhost/*.localhost/file:// + TONONE_JEV_ULTRAFAST_HOSTS, production refused — @fatih
2026-09-20 23:51 : trap kept from eval — lib/jev accepts JEV_API_KEY or TYPESAFE_API_KEY, jev-ultrafast reads only TYPESAFE_API_KEY; cli.py child_env bridges it in the subprocess only, never persisted — @fatih
2026-09-20 23:51 : jev-ultrafast status vocabulary is only done/blocked (no needs_login/ambiguous/stuck like jev-browser) — skill derives those from transcript: stuck on repeated page_changed=false, contested target on low probability, auth precondition when flow never leaves /login — @fatih
2026-09-20 23:51 : skills 426 → 427, index + mirror regenerated, 69 pytest + 4 node pass, CHANGELOG Unreleased — all uncommitted — @fatih
2026-09-24 00:36 : absorbed Anthropic "Getting the most out of Opus 5.5" post (2026-09-22) — apex.md new Long Runs section (stop rules, TASKS.md for M+, subagent evidence check, blockers-first summary), apex-review findings need file:line + failure scenario, prompt-design/prompt agent drop manual CoT for thinking models, cite-*/crest-compete mark unconfirmed — @fatih
[!!] 2026-09-24 00:36 : Opus 5.5 rule — never prompt model to reproduce internal reasoning in reply, can trip bio/cyber safety flags and auto-switch to older model; grep found zero such prompts in repo, only one "think hard" (helm-brief, removed) — @fatih
2026-09-24 00:36 : CLAUDE.md gained "Working in this repo" section; CHANGELOG Unreleased entry; 41 structure/compliance tests pass; all uncommitted, awaiting commit/PR/version-bump call — @fatih
2026-09-24 00:40 : reviewed every-app/open-seo (MIT, ~20k stars) — value is 10 SKILL.md in plugins/openseo/skills (seo-audit, keyword-research, keyword-clustering, link-prospecting, local-seo, competitor-analysis, competitive-landscape, seo-report, seo-coach, seo-project-setup), app = DataForSEO wrapper + MCP — @fatih
[!!] 2026-09-24 00:40 : gap found: ink-cluster + ink-seo ask for "Estimated MSV [X]" with no data source (ink-cluster lacks WebSearch) = invites fabricated metrics; open-seo rule: no tool value, write unknown — @fatih
2026-09-24 00:40 : proposed, no code yet — rewrite ink-seo (shortlist 5-10 then 1-3 recs, "what else we checked", honest sizing, rank conventions, protect winners), ink-cluster by SERP overlap + cannibalization, new ink-links + ink-local, vendor-neutral SEO-MCP detect with WebSearch fallback, ledger in docs/upstream.md — awaiting go + AI-visibility scope call — @fatih
2026-09-24 00:58 : implemented open-seo steal on branch feat/ink-seo-evidence — ink-seo rewritten (data tier A/B/C, shortlist→1-3 recs, "what else we checked", honest sizing, rank conventions), ink-cluster by SERP overlap + evidence-only cannibalization, ink-brief no guessed volume, new ink-links + ink-local, shared .tonone/seo/{context,research-log}.md — @fatih
2026-09-24 00:58 : skills 427 → 429, mirror + index + bundle/marketing-team symlinks + README/CLAUDE/repomap counts, upstream.json pins open-seo v0.1.9, ledger entry, CHANGELOG Unreleased, 41 tests pass — uncommitted, AI visibility deferred — @fatih
[!!] 2026-09-24 00:58 : trap: colon-space inside unquoted SKILL.md description ("Evidence-first: metrics") breaks YAML frontmatter, fails 5 compliance tests — use em dash — @fatih
