...
cloudrun-agent --history 2>/dev/null || uvx cloudrun-agent -- --history 2>/dev/null || "$PLUGIN_ROOT/scripts/.venv/bin/python" -m cloudrun_agent.cli --history 2>/dev/null || uv run --directory "$PLUGIN_ROOT/scripts" python -m cloudrun_agent.cli --history
```
