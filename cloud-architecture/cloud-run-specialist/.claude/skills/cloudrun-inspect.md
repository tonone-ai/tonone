...
```bash
cloudrun-agent analyze --list 2>/dev/null || uv run python -m cloudrun_agent.cli --list
```
...
```bash
cloudrun-agent analyze --service SERVICE_NAME --region REGION 2>/dev/null || uv run python -m cloudrun_agent.cli --service SERVICE_NAME --region REGION
```
