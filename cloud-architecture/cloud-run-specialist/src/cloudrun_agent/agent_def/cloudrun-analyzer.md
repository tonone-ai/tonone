...
Also verify the analyzer is available:
...
Use whichever command succeeds as the base command for all subsequent calls. Store it mentally as CMD.
...
```bash
CMD --service SERVICE_NAME --region REGION
```
